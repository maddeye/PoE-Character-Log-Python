    ...
    for item in items:
        ...
        if (
            ...
            if "socketedItems" in item:
                for gem in range(len(item["socketedItems"])):
                    gqual = "0"
                    glvl = "1"
...
def make_xml(character_history: List[dict]) -> str:
    ...
    # For each level
    for e in range(len(character_history)):
        ...
        gemgroups = buildskills(character_items)
        mainskills = []
        for slot in gemgroups:
            ...
            for group in gemgroups[slot]:
                if len(group["supports"]) > 0:
                    for gm in group["gems"]:
                        mainskills.append(
                            "[" + str(len(group["supports"])) + "] " + getname(gm)
                        )
                if len(group["gems"]) > 0:
            ...
            for itm in character_items:
                ...
                if itemkey in itemdb:
                    ...
                    if "sockets" in itm:
                        itemtext += "Sockets: "
                        for gem in range(len(itm["sockets"])):
                            if (
                                gem > 0
...
def pob_code_from_char(char: dict) -> str:
    return pob_code(make_xml([char]))
